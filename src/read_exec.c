...
int to_hex2(char a, char b) { return to_hex(a) * 16 + to_hex(b); }
...
int read_exec(struct memory* mem, struct assembly* as, const char* name,
              FILE* log_file) {
  FILE* fp;
  fp = fopen(name, "r");
  if (fp == NULL) {
  ...
  while (fgets(line, MAXLINE, fp)) {
    ...
    int          num_hex = 0;
    int n = sscanf(line, " %x %s%s%s%s*c[ ]*c[]", &addr, hexes[0], hexes[1],
                   hexes[2], hexes[3]);
    if (n >= 2) {
    ...
    } else if ((n = sscanf(line, " %x: %x %7s %15s %23s", &addr, &a, opcode,
                           args, rest)) >= 2) {
      msg = "Insn";
